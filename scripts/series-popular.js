...
async function fetchMovies(category, limit = 10) {
    try {
        ...
        const response = await fetch(`${API_BASE_URL}/${category}?api_key=${API_KEY}&language=fr-FR&page=1`);
        if (!response.ok) {
            throw new Error(`Erreur lors de la récupération des séries: ${response.statusText}`);
        }
...
// Fonction pour créer le slider des séries populaires
async function createPopularSerieslider() {
    const category = "tv/popular"; // Catégorie TMDB pour les séries populaires
    ...
    series.forEach((serie) => {
        ...
        const releaseDate = serie.first_air_date ? ` ${serie.first_air_date}` : "Date inconnue";  // Date de première diffusion
    ...
    // Initialisation du Swiper pour les séries populaires
...
document.addEventListener("DOMContentLoaded", async () => {
    // 1️⃣ Récupérer et afficher les séries populaires
    await createPopularSerieslider();
